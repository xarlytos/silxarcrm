  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3.5">
        <div className="relative shrink-0 w-10 h-10 rounded-xl bg-[var(--bg-tertiary)] ring-1 ring-cyan-400/25 flex items-center justify-center"
             style={{ boxShadow: 'inset 0 0 14px rgba(34,211,238,0.18)' }}>
          <Icon className="w-5 h-5 text-cyan-500" strokeWidth={1.8} />
        </div>
        <div className="flex-1 min-w-0 leading-tight">
          <p className="text-[9.5px] uppercase tracking-[0.16em] font-semibold text-cyan-500/85">Próximo objetivo</p>
          <p className="text-[14px] font-bold text-[var(--text-primary)] leading-tight mt-0.5 truncate">{tarea.titulo}</p>
          <p className="text-[11.5px] text-[var(--text-secondary)] leading-snug truncate">{tarea.descripcion}</p>
        </div>
        <div className="text-right shrink-0 hidden sm:flex flex-col">
          <p className="text-[8.5px] uppercase tracking-wider text-[var(--text-tertiary)]">Te faltan</p>
          <p className="text-[22px] font-black text-cyan-500 leading-none tabular-nums">{restantes}</p>
          <p className="text-[9px] text-[var(--text-tertiary)] font-mono">de {tarea.meta}</p>
        </div>
        <ChevronRight className="w-4 h-4 text-cyan-500/70 shrink-0 group-hover:translate-x-1 transition-transform" />
      </div>
