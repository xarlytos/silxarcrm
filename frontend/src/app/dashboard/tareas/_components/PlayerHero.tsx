      ...
      <div className="relative grid grid-cols-1 md:grid-cols-[auto_1fr_auto] gap-5 items-center">
        ...
        <div className="min-w-0 space-y-3">
          ...
          <div className="flex items-baseline gap-x-3 gap-y-1 flex-wrap">
            <h1 className="text-[24px] md:text-[28px] font-black tracking-tight text-white leading-none">
              {titleInfo.title}
            </h1>
            <span className="text-[10.5px] uppercase tracking-[0.2em] font-bold text-amber-400/90">
              Nivel {level}
            </span>
          </div>
          ...
          <div className="space-y-2">
            <div className="relative">
              {/* Track */}
              <div className="relative h-2.5 rounded-full bg-black/50 ring-1 ring-white/[0.04] overflow-hidden">
                {/* Fill */}
                <div
                ...
                {milestones.map((m, i) => {
                  ...
                  return (
                    <div key={i} className="absolute -translate-x-1/2 -translate-y-1/2" style={{ left: `${m.pct * 100}%`, top: '50%' }}>
                      <div className={`w-2 h-2 rotate-45 ${reached ? 'bg-amber-300' : 'bg-white/15'} ${reached ? 'shadow-[0_0_8px_rgba(251,191,36,0.6)]' : ''}`} />
                    </div>
                  );
                })}
              </div>
            ...
            <div className="flex items-center justify-between text-[11px]">
              <span className="font-mono text-white/75">
                <span className="text-amber-300 font-bold">{xpInLevel}</span>
                <span className="text-white/40"> / {xpForNext} XP</span>
              </span>
              <div className="hidden sm:flex items-center gap-3">
                {milestones.slice(0, 3).map((m, i) => (
                  <span key={i} className={`text-[10px] font-mono ${progress >= m.pct ? 'text-amber-300' : 'text-white/30'}`}>
                    {m.xp} XP
                  </span>
                ))}
                <span className={`text-[10px] font-mono ${progress >= 1 ? 'text-amber-300' : 'text-white/30'}`}>
                  {xpForNext} XP
                </span>
              </div>
              <span className="text-white/40">
                Total: <span className="font-mono text-violet-300 font-bold">{totalXp.toLocaleString()}</span>
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <div className={`inline-flex items-center gap-2 px-2.5 h-7 rounded-lg ${
              buff.level > 0 ? 'bg-orange-500/10 ring-1 ring-orange-500/25 text-orange-300' : 'bg-white/[0.03] ring-1 ring-white/10 text-white/40'
            }`}>
        ...
        <div className="grid grid-cols-3 md:grid-cols-1 gap-2 md:w-[170px] shrink-0">
          <CompactStat icon={Flame} value={streak} label={`Racha · máx ${bestStreak}`} color="text-orange-400" iconBg="bg-orange-500/10" ring="ring-orange-500/25" />
          <CompactStat icon={Gem} value={totalGemas.toLocaleString()} label="Gemas" color="text-cyan-300" iconBg="bg-cyan-500/10" ring="ring-cyan-500/25" />
          <CompactStat icon={Trophy} value={`${unlocked}/${totalAch}`} label="Logros" color="text-violet-300" iconBg="bg-violet-500/10" ring="ring-violet-500/25" />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className={`flex items-center gap-2.5 px-2.5 h-12 rounded-xl ${iconBg} ring-1 ${ring}`}>
      ...
      <div className="min-w-0 leading-tight">
        ...
        <p className="text-[9px] uppercase tracking-[0.12em] font-semibold text-white/45 truncate">{label}</p>
      </div>
    </div>
  );
}
...
function HexagonAvatar({ level }: { level: number }) {
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      ...
      {/* Star pip */}
      <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-7 h-7 rounded-full bg-amber-400 ring-2 ring-[#0a0a0d] flex items-center justify-center shadow-lg shadow-amber-500/40">
